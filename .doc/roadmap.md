...
- [ ] **10.1. Настройка куков Kwork (`KWORK_COOKIES`)**
  - [ ] Извлечь Cookie-строку авторизованной сессии из браузера на kwork.ru
...
- [ ] **11.1. Генерация авто-откликов через ИИ (`llm.py`)**
  - [ ] Добавить асинхронную функцию `async def generate_cover_letter(order_title: str, order_description: str) -> str`
  - [ ] Сформировать промпт Cohere AI для создания короткого, продающего и убедительного сопроводительного письма на основе `MY_SKILLS`
  - [ ] Прикреплять сгенерированное сопроводительное письмо прямо в Telegram-сообщение для копирования в 1 клик

- [ ] **11.2. Inline-кнопки быстрого отклика в Telegram (`notifier.py`)**
  - [ ] Добавить `InlineKeyboardMarkup` и `InlineKeyboardButton` под каждым отправленным заказом
  - [ ] Настроить кнопку `[ 🚀 Откликнуться на {source} ]` с прямой URL-ссылкой на заказ
