...
- [ ] **9.1. Реальный парсинг Kwork (`exchanges/kwork.py`)**
  - [ ] Реализовать HTTP-запрос к ленте проектов Kwork (`/projects`) через `aiohttp` с `KWORK_COOKIES`
  - [ ] Настроить извлечение заголовка, описания, бюджета, срока, автора и ссылки на проект через `BeautifulSoup`

- [ ] **9.2. Реальный парсинг Яндекс.Услуг (`exchanges/yandex_uslugi.py`)**
  - [ ] Реализовать HTTP-запрос к заказам Яндекс.Услуг с `YANDEX_COOKIES`
  - [ ] Настроить парсинг данных заказа и маппинг в объекты `Order`

- [ ] **9.3. Автоматизация опроса 24/7 через `cron-job.org`**
  - [ ] Зарегистрировать Cron-задание на `cron-job.org` с вызовом `https://kworkbot-mu.vercel.app/`
  - [ ] Установить периодичность выполнения каждые 1–5 минут

- [ ] **9.4. Тонкая настройка фильтрации и навыков**
  - [ ] Настроить детальный список `MY_SKILLS` в `.env` и Vercel
  - [ ] Оптимизировать промпт оценки Cohere AI для исключения ложных срабатываний

- [ ] **9.5. Масштабирование (Подключение дополнительных бирж)**
  - [ ] Разработать модуль парсинга FL.ru (`exchanges/fl_ru.py`)
  - [ ] Разработать модуль парсинга Хабр Фриланс (`exchanges/habr_freelance.py`)
  - [ ] Зарегистрировать новые биржи в `EXCHANGES` в `config.py`
