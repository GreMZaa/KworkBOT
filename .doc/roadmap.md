...
## 🛡 Этап 12: Telegram Команды управления (/skills, /threshold, /stats) & Антискам-фильтр

- [x] **12.1. Хранение пользовательских настроек и статистики в БД (`db.py`)**
  - [x] Создать таблицу `settings (key TEXT PRIMARY KEY, value TEXT)`
  - [x] Создать функции `get_setting(key, default)`, `set_setting(key, value)`
  - [x] Создать функцию `get_stats_summary()` (всего заказов за 24 часа, по биржам, средняя релевантность)

- [x] **12.2. Команды Telegram-бота (`main.py` & `api/index.py`)**
  - [x] Реализовать команду `/stats` (красивая статистика заказов)
  - [x] Реализовать команду `/skills` (просмотр и смена `MY_SKILLS` прямо в чате)
  - [x] Реализовать команду `/threshold [число]` (динамическая смена порога отбора)

- [x] **12.3. Умный антискам-фильтр (`llm.py` & `notifier.py`)**
  - [x] Добавить функцию `check_is_scam(title, description) -> tuple[bool, str]`
  - [x] Добавить детектор стоп-фраз мошенников (залоги, внешние мессенджеры, бесплатные тестовые)
  - [x] Подсвечивать подозрительные заказы плашкой ⚠️ **ВНИМАНИЕ: ВОЗМОЖЕН СКАМ!**
